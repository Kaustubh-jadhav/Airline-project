		...
		<?php
			if(isset($_POST['Cancel_Ticket']))
			{
				...
				if(empty($data_missing))
				{
					...
					$query="SELECT count(*) from ticket_details t WHERE pnr='$pnr' and journey_date>='$todays_date'";
				...
				//	echo "hello";die;
					...
					$query="UPDATE ticket_details SET booking_status='CANCELED' WHERE pnr='$pnr' and customer_id='$customer_id'";
					$stmt=mysqli_prepare($dbc,$query);
					...
					if($affected_rows==1)
					{
						$query="SELECT t.flight_no,t.journey_date,t.no_of_passengers,t.class,0.85*p.payment_amount as refund_amount from ticket_details t,payment_details p WHERE t.pnr='$pnr' and t.pnr=p.pnr";
						$stmt=mysqli_prepare($dbc,$query);
