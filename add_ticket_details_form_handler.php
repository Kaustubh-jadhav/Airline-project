			...
			if(isset($_POST['Submit']))
			{
				...
				*/
				...
				for($i=1;$i<=$no_of_pass;$i++)
				{
					...
					{
						$name1 = $_POST['pass_name'][$i-1];
						$id1 = $_POST['pass_ff_id'][$i-1];
						$query="SELECT count(*) from customer c, frequent_flier_details f WHERE c.name='$name1' and f.frequent_flier_no='$id1' and c.customer_id=f.customer_id";
						$stmt=mysqli_prepare($dbc,$query);
					//	mysqli_stmt_bind_param($stmt,"ss",$_POST['pass_name'][$i-1],$_POST['pass_ff_id'][$i-1]);
						...
						if($cnt==1)
						{
							$ffid=$_POST['pass_ff_id'][$i-1];
							$query="UPDATE frequent_flier_details SET mileage=mileage+'$ff_mileage' where frequent_flier_no='$ffid'";
							$stmt=mysqli_prepare($dbc,$query);
					...
					$p_name=$_POST['pass_name'][$i-1];
					$p_age=$_POST['pass_age'][$i-1];
					$p_gender=$_POST['pass_gender'][$i-1];
					$p_meal=$_POST['pass_meal'][$i-1];
					$p_id=$_POST['pass_ff_id'][$i-1];
					$query="INSERT INTO passengers (passenger_id,pnr,name,age,gender,meal_choice,frequent_flier_no) VALUES ('$i','$pnr','$p_name','$p_age','$p_gender','$p_meal','$p_id')";
					$stmt=mysqli_prepare($dbc,$query);
